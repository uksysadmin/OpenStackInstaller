...
nova_install () {
	apt-get -y install nova-compute nova-network nova-api python-keystone
	configure_nova
}

extras_install() {
	apt-get -y install euca2ools unzip qemu ntp python-dateutil
}

mysql_install() {
	# Read config
	# Decide on whether this refers to installing MySQL first
	# Or just configuration against a database service that exists already
	
	# Take MYSQL_ADDR and compare to an address on this box. If it matches, install + configure.
	# If not, configure only
	is_local=$(ip addr | grep 
}
